...
def update(status_code, output, data):
    ...
    if res:
        status_code, output = api.update_obj('api/system_address', data, payload)
...
def create(data):
    ...
    payload = {
        'mkey': data['name'],
        'ip-netmask': data['ip-netmask']
    }
    status_code, output = api.add_obj('api/system_address', data, payload)
...
def add_address(data):
    ...
    data['mkey'] = data['name']
    status_code, output = api.get_obj('api/system_address', data)
...
def param_check(module):
    ...
    if not module.params['name']:
        ...
        res = False
    if not module.params['ip-netmask']:
        err_msg.append('Parameter `ip-netmask` must be set')
        res = False
...
def main():
    fields = {
        ...
        "ip-netmask": {"required": False, "type": "str", "default": ""}
    }

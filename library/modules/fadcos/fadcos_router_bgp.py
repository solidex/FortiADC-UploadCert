...
def update(status_code, output, data):
    ...
    if res:
        data['mkey'] = '-1'
        status_code, output = api.update_obj('api/router_bgp', data, payload)
...
def create(data):
    ...
    status_code, output = api.add_obj('api/router_bgp', data, payload)
...
def config_router(data):
    ...
    data['mkey'] = "-1"
    status_code, output = api.get_obj('api/router_bgp', data)
...
def param_check(module):
    ...
    if not module.params['router-id']:
        err_msg.append('Parameter `router-id` must be set')
        res = False
    if not module.params['as']:
...
def main():
    fields = {
        ...
        "router-id": {"required": True, "type": "str"},

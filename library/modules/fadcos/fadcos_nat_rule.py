...
def update(status_code, output, data):
    ...
    if res:
        status_code, output = api.update_obj('api/firewall_nat_snat', data, payload)
...
def create(data):
    ...
    payload = {
        ...
        "to": data['to'],
        "trans-to-ip": data['trans-to-ip'],
        "trans-to-ip-start": "0.0.0.0",
        "trans-to-ip-end": "0.0.0.0",
        "trans-to-type": data['trans-to-type'],
        "status": "enable"
    }
    status_code, output = api.add_obj('api/firewall_nat_snat', data, payload)
...
def add_rule(data):
    ...
    data['mkey'] = data['name']
    status_code, output = api.get_obj('api/firewall_nat_snat', data)
...
def main():
    fields = {
        ...
        "out-interface": {"required": True, "type": "str"},
        "to": {"required": True, "type": "str"},
        "trans-to-ip": {"required": True, "type": "str"},
        "trans-to-type": {"required": False, "type": "str", "default": "ip"}
    }
